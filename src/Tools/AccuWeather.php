            ...
            $message = [
                ...
                'fields' => [
                    [
                        ...
                        'value' => 'Min: ' . $this->convertFoC($forecasts[0]->Temperature->Minimum->Value) . ' | Max: ' . $this->convertFoC($forecasts[0]->Temperature->Maximum->Value) . ' | Ngày: ' . $forecasts[0]->Day->IconPhrase . ' | Đêm: ' . $forecasts[0]->Night->IconPhrase,
                        'short' => true,
                    ],
                    [
                        'title' => date('Y-m-d', $forecasts[1]->EpochDate),
                        'value' => 'Min: ' . $this->convertFoC($forecasts[1]->Temperature->Minimum->Value) . ' | Max: ' . $this->convertFoC($forecasts[1]->Temperature->Maximum->Value) . ' | Ngày: ' . $forecasts[1]->Day->IconPhrase . ' | Đêm: ' . $forecasts[1]->Night->IconPhrase,
                        'short' => true,
                    ],
                    [
                        'title' => date('Y-m-d', $forecasts[2]->EpochDate),
                        'value' => 'Min: ' . $this->convertFoC($forecasts[2]->Temperature->Minimum->Value) . ' | Max: ' . $this->convertFoC($forecasts[2]->Temperature->Maximum->Value) . ' | Ngày: ' . $forecasts[2]->Day->IconPhrase . ' | Đêm: ' . $forecasts[2]->Night->IconPhrase,
                        'short' => true,
                    ],
                    [
                        'title' => date('Y-m-d', $forecasts[3]->EpochDate),
                        'value' => 'Min: ' . $this->convertFoC($forecasts[3]->Temperature->Minimum->Value) . ' | Max: ' . $this->convertFoC($forecasts[3]->Temperature->Maximum->Value) . ' | Ngày: ' . $forecasts[3]->Day->IconPhrase . ' | Đêm: ' . $forecasts[3]->Night->IconPhrase,
                        'short' => true,
                    ],
                    [
                        'title' => date('Y-m-d', $forecasts[4]->EpochDate),
                        'value' => 'Min: ' . $this->convertFoC($forecasts[4]->Temperature->Minimum->Value) . ' | Max: ' . $this->convertFoC($forecasts[4]->Temperature->Maximum->Value) . ' | Ngày: ' . $forecasts[4]->Day->IconPhrase . ' | Đêm: ' . $forecasts[4]->Night->IconPhrase,
                        'short' => true,
                    ],
                ],
            ];
